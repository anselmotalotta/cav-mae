...
class ImagePatchEmbed(PatchEmbed):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        """
        PatchEmbed module for transforming image into a sequence of patches.

        Args:
            img_size (int or tuple): Size of the input image. Default is 224.
            patch_size (int or tuple): Size of each patch. Default is 16.
            in_chans (int): Number of input channels. Default is 3.
            embed_dim (int): Dimension of the embedded patches. Default is 768.
        """
        super().__init__(img_size, patch_size, in_chans, embed_dim)

    def forward(self, x):
        """
        Forward pass of the PatchEmbed module. Like the parent class but with relaxed size constraints.
...
class AudioPatchEmbed(nn.Module):
    def __init__(self, audio_length, patch_size=16, in_chans=1, embed_dim=768):
        """
        Initializes the AudioPatchEmbed module.

        Args:
            audio_length (int): The length of the audio signal.
            patch_size (int or tuple): The size of the patches to be extracted from the audio signal. Default is 16.
            in_chans (int): The number of input channels. Default is 1.
            embed_dim (int): The dimension of the embedded representation. Default is 768.
        """
        super().__init__()
        patch_size = to_2tuple(patch_size)
        self.patch_size = patch_size
        self.num_patches = int(audio_length / 2)

        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)

    def forward(self, x):
        """
        Forward pass of the AudioPatchEmbed module.

        Args:
            x (torch.Tensor): The input audio signal tensor.

        Returns:
            torch.Tensor: The embedded representation of the input audio signal.
        """
        x = self.proj(x).flatten(2).transpose(1, 2)
        return x
...
class CustomBlock(Block):
    """
    Block module in the CAM-MAE model. Like parent class but with support to Video andAudio Norm layers

    Args:
        dim (int): The input dimension of the block.
        num_heads (int): The number of attention heads.
        mlp_ratio (float, optional): The ratio of hidden dimension to input dimension in the MLP. Default is 4.0.
        qkv_bias (bool, optional): Whether to include bias terms in the query, key, and value projections. Default is False.
        qk_scale (float, optional): Scale factor for query and key. Default is None.
        ...
        drop_path (float, optional): Stochastic depth rate for drop path. Default is 0.0.
    ...
    def forward(self, x, modality=None):
        ...
        Args:
            ...
            modality (str, optional): The modality of the input tensor. Default is None.
...
class CAMMAE(nn.Module):
    ...
    def __init__(self, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
        ...
        super().__init__()
        print('A CAM-MAE Model')
        print('Use norm_pix_loss: ', norm_pix_loss)
        print('Learnable Positional Embedding: ', tr_pos)

        # the encoder part

        self.patch_embed_a = AudioPatchEmbed(audio_length, patch_size, 1, embed_dim)
        self.patch_embed_v = ImagePatchEmbed(img_size, patch_size, in_chans, embed_dim)

        print(f'Number of Audio Patches: {self.patch_embed_a.num_patches:d}, Visual Patches: {self.patch_embed_v.num_patches:d}')
        ...
        # audio-branch
        self.blocks_a = nn.ModuleList([CustomBlock(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer) for i in range(modality_specific_depth)])
        # visual-branch
        self.blocks_v = nn.ModuleList([CustomBlock(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer) for i in range(modality_specific_depth)])
        # unified branch
        self.blocks_u = nn.ModuleList([CustomBlock(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer) for i in range(12-modality_specific_depth)])
        ...
        self.decoder_blocks = nn.ModuleList([CustomBlock(decoder_embed_dim, decoder_num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer) for i in range(decoder_depth)])
...
# the finetuned CAM-MAE model
class CAMMAEFT(nn.Module):
